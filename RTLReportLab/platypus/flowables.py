...
from RTLReportLab.lib.colors import gray, lightgrey
from RTLReportLab.lib.rl_accel import fp_str
from RTLReportLab.lib.enums import TA_LEFT, TA_CENTER, TA_RIGHT
from RTLReportLab.lib.styles import _baseFontName
from RTLReportLab.lib.utils import strTypes, rl_safe_exec, annotateException
from RTLReportLab.lib.abag import ABag
from RTLReportLab.pdfbase import pdfutils
from RTLReportLab.pdfbase.pdfmetrics import stringWidth
from RTLReportLab.rl_config import _FUZZ, overlapAttachedSpace, ignoreContainerActions, listWrapOnFakeWidth
...
class Image(Flowable):
    ...
    def __init__(self, filename, width=None, height=None, kind='direct',
        ...
        if not fp and os.path.splitext(filename)[1] in ['.jpg', '.JPG', '.jpeg', '.JPEG']:
            ...
            from RTLReportLab.lib.utils import open_for_read
    ...
    def __getattr__(self,a):
        if a=='_img':
            from RTLReportLab.lib.utils import ImageReader  #this may raise an error
...
class CondPageBreak(Spacer):
    ...
    def wrap(self, availWidth, availHeight):
        if availHeight<self.height:
            ...
            from RTLReportLab.platypus.doctemplate import FrameBreak
...
def _listWrapOn(F,availWidth,canv,mergeSpace=1,obj=None,dims=None,fakeWidth=None):
    ...
    if cframe:
        from RTLReportLab.platypus.doctemplate import _addGeneratedContent, Indenter
        doct_frame = cframe
    ...
    try:
        ...
        while F:
            ...
            if hasattr(f,'frameAction'):
                from RTLReportLab.platypus.doctemplate import Indenter
...
def _flowableSublist(V):
    ...
    from RTLReportLab.platypus.doctemplate import LCActionFlowable
...
class KeepTogether(_ContainerSpace,Flowable):
    ...
    def __init__(self,flowables,maxHeight=None):
        if not hasattr(KeepTogether,'NullActionFlowable'):
            ...
            from RTLReportLab.platypus.doctemplate import NullActionFlowable
            from RTLReportLab.platypus.doctemplate import FrameBreak
...
class _Container(_ContainerSpace):  #Abstract some common container like behaviour
    def drawOn(self, canv, x, y, _sW=0, scale=1.0, content=None, aW=None):
        ...
        from RTLReportLab.platypus.doctemplate import ActionFlowable, Indenter
        x0 = x
        y0 = y
        pS = 0
...
class KeepInFrame(_Container,Flowable):
    ...
    def wrap(self,availWidth,availHeight):
        from RTLReportLab.platypus.doctemplate import LayoutError
        mode = self.mode
    ...
    def _findSplit(self,canv,availWidth,availHeight,mergeSpace=1,obj=None,content=None,paraFix=True):
        ...
        for i,f in enumerate(F):
            ...
            if H>availHeight:
                aH = availHeight-(H-h)
                if paraFix:
                    from RTLReportLab.platypus.paragraph import Paragraph
...
class BalancedColumns(_FindSplitterMixin,NullDraw):
    ...
    def _generated_content(self,aW,aH):
        G = []
        frame = self._frame
        from RTLReportLab.platypus.doctemplate import LayoutError, ActionFlowable, Indenter
        from RTLReportLab.platypus.frames import Frame
        from RTLReportLab.platypus.doctemplate import FrameBreak
...
class FrameBG(AnchorFlowable):
    ...
    def __init__(self, color=None, left=0, right=0, start=True, strokeWidth=None, strokeColor=None, strokeDashArray=None):
        ...
        self.start = start
        if start:
            from RTLReportLab.platypus.doctemplate import _evalMeasurement
...
class FrameSplitter(NullDraw):
    ...
    def wrap(self,aW,aH):
        frame = self._frame
        from RTLReportLab.platypus.doctemplate import NextPageTemplate,CurrentFrameFlowable,LayoutError
...
from RTLReportLab.lib.sequencer import _type2formatter
_bulletNames = dict(
...
from RTLReportLab.lib.styles import ListStyle
class ListItem:
    def __init__(self,
...
class DocPara(DocAssign):
    ...
    def wrap(self,aW,aH):
        ...
        P = self.klass
        if not P:
            from RTLReportLab.platypus.paragraph import Paragraph as P
        style = self.style
        if not style:
