...
from RTLReportLab.pdfbase import _fontdata
from RTLReportLab.lib.logger import warnOnce
from RTLReportLab.lib.utils import rl_isfile, rl_glob, rl_isdir, open_and_read, open_and_readlines, findInPaths, isSeq, isStr
from RTLReportLab.rl_config import defaultEncoding, T1SearchPath
from RTLReportLab.lib.rl_accel import unicode2T1, instanceStringWidthT1
from RTLReportLab.pdfbase import rl_codecs
_notdefChar = b'n'
...
def bruteForceSearchForFile(fn,searchPath=None):
    if searchPath is None: from RTLReportLab.rl_config import T1SearchPath as searchPath
        ...
        for i in range(256):
            ...
            if glyphName is not None:
                try:
                    ...
                    import RTLReportLab.rl_config
                    if RTLReportLab.rl_config.warnOnMissingFontGlyphs:
    ...
    def _loadMetrics(self, afmFileName):
        ...
        if topLevel.get('EncodingScheme', None) == 'FontSpecific':
            ...
            if _postScriptNames2Unicode is None:
                try:
                    from RTLReportLab.pdfbase._glyphlist import _glyphname2unicode
                    _postScriptNames2Unicode = _glyphname2unicode
                    del _glyphname2unicode
                except:
                    _postScriptNames2Unicode = {}
                    raise ValueError(
                            "cannot import module RTLReportLab.pdfbase._glyphlist module\n"
...
def registerFontFamily(family,normal=None,bold=None,italic=None,boldItalic=None):
    from RTLReportLab.lib import fonts
