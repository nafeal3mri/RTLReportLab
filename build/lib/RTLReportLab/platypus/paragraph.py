...
from RTLReportLab.pdfbase.pdfmetrics import stringWidth, getAscentDescent, getFont
from RTLReportLab.pdfgen.textobject import rtlSupport, bidiText, bidiWordList, isBidiStr,\
                                        bidiStrWrap, isBidiList, BidiList,\
                                        bidiListWrap, BidiIndex, bidiFragWord,\
                                        innerBidiStrWrap
from RTLReportLab.pdfbase.ttfonts import shapeFragWord
from RTLReportLab.platypus.paraparser import ParaParser, _PCT, _num as _parser_num, _re_us_value
from RTLReportLab.platypus.flowables import Flowable
from RTLReportLab.lib.colors import Color
from RTLReportLab.lib.enums import TA_LEFT, TA_RIGHT, TA_CENTER, TA_JUSTIFY
from RTLReportLab.lib.geomutils import normalizeTRBL
from RTLReportLab.lib.textsplit import wordSplit, ALL_CANNOT_START
from RTLReportLab.lib.styles import ParagraphStyle
from copy import deepcopy
from RTLReportLab.lib.abag import ABag
from RTLReportLab.rl_config import decimalSymbol, _FUZZ, paraFontSizeHeightOffset,\
    hyphenationMinWordLength
from RTLReportLab.lib.utils import _className, isBytes, isStr
from RTLReportLab.lib.rl_accel import sameFrag
import re
...
def _BidiStrWrapRejoinSplitWords(R):
    s = _rejoinSplitWords(R)
    orig = R[0]
    # Guard: orig must have bidi attrs (plain _SplitWord from a Latin run won't)
    if hasattr(orig, '__bidiL__') and hasattr(orig, '__bidiV__'):
        return bidiStrWrap(s, orig)
    return s
...
def _split_blParaSimple(blPara,start,stop):
    ...
    if isinstance(blPara.lines,BidiList):
        # Use getattr fallback so plain strings from mixed-script runs don't crash
        f.words.sort(key = lambda _: getattr(_, '__bidiL__', 0))
...
class Paragraph(Flowable):
    """ Paragraph(text, style, bulletText=None, caseSensitive=1)
        ...
        style is a style definition as in RTLReportLab.lib.styles.
        ...
        of 4 fonts using RTLReportLab.pdfbase.pdfmetrics.registerFont; then
        use the addMapping function to tell the library that these 4 fonts
        form a family e.g.
        from RTLReportLab.lib.fonts import addMapping
    ...
    def breakLines(self, width):
        ...
        if (nFrags==1
            ...
            # Apply character-level bidi reordering and/or HarfBuzz shaping.
            # Word-level visual ordering is handled by [::-1] in drawPara for RTL.
            if doBidi and _words and not any((isBidiList(_) for _ in _words)):
                if shaping:
                    _words = [shapeFragWord(_) for _ in _words]
                else:
                    _words = [bidiFragWord(_,direction=wordWrap) for _ in _words]
            elif shaping:
                _words = [shapeFragWord(_) for _ in _words]
            bidiSort = False
            ...
            while _words:
                ...
                if not endLine:
                    ...
                    if not words:
                        ...
                        if wordWrap == 'RTL':
                            # RTL: prepend new word so visual order is correct
                            # (bidi-processed words must appear right-to-left in the string)
                            g.text = nText + ' ' + g.text
        ...
        if nLines > 0:
            ...
            if blPara.kind==0:
                if alignment == TA_LEFT:
                    # RTL paragraphs default to right-aligned starts
                    dpl = _rightDrawParaLine if isRTL else _leftDrawParaLine
                ...
                if alignment == TA_LEFT:
                    # RTL paragraphs default to right-aligned starts
                    dpl = _rightDrawParaLineX if isRTL else _leftDrawParaLineX
                elif alignment == TA_CENTER:
...
if __name__=='__main__':    #NORUNTESTS
    ...
    def dumpProcessedFrags(P,label='processed_frags'):
        if isinstance(P2.frags[0],list):
            ...
            for _ in P2.frags:
                _S(_showWord(_))
            print('from RTLReportLab.platypus.paragraph import _HSFrag, _SplitFragHS, _SplitFragHY, _SplitFrag, _getFragWords\nfrom RTLReportLab.platypus.paraparser import ParaFrag\nfrom RTLReportLab.lib.colors import Color')
            print('\n'.join(('__frag_%s__ = %r' % _ for _ in sorted(_F.values()))))
            print('%s=[\n%s\x20\x20]' % (processed_frags,'\n'.join(_S.__self__)))
            print('print(_getFragWords(processed_frags))')


    from RTLReportLab.lib.styles import getSampleStyleSheet, ParagraphStyle
    from RTLReportLab.lib.units import cm
    import sys
    TESTS = sys.argv[1:]
    if TESTS==[]: TESTS=['4']
...
It should be a normal<onDraw name="_indexAdd" label="normal"/> paragraph, set in Courier 12 pt."""
    ...
    if flagged(12):
        ...
        from RTLReportLab.pdfbase.pdfmetrics import registerFont
