...
from RTLReportLab.lib.rltempfile import get_rl_tempfile, get_rl_tempdir
...
import RTLReportLab
__RL_DIR=os.path.dirname(RTLReportLab.__file__)    #possibly relative
...
def getHyphenater(hDict=None):
    try:
        from RTLReportLab.lib.pyHnj import Hyphen
...
def open_for_read(name,mode='b'):
    ...
    def datareader(url, unquote=unquote):
        ...
        return data
    from RTLReportLab.rl_config import trustedHosts, trustedSchemes
    if trustedHosts:
        import re, fnmatch
        def xre(s):
...
class ImageReader:
    ...
    def __init__(self, fileName,ident=None):
        ...
        if _isPILImage(fileName):
            ...
            try:
                from RTLReportLab.rl_config import imageReaderFlags
...
class DebugMemo:
    ...
    def __init__(self,fn='rl_dbgmemo.dbg',mode='w',getScript=1,modules=(),capture_traceback=1, stdout=None, **kw):
        ...
        for n,m in sys.modules.items():
            if n=='reportlab' or n=='rlextra' or n[:10]=='RTLReportLab.' or n[:8]=='rlextra.':
...
class TimeStamp:
    def __init__(self,invariant=None):
        if invariant is None:
            from RTLReportLab.rl_config import invariant
