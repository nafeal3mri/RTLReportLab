...
"""
from RTLReportLab.lib.colors import Color, CMYKColor, CMYKColorSep, toColor
from RTLReportLab.lib.utils import isBytes, isStr, asUnicode
from RTLReportLab.lib.rl_accel import fp_str
from RTLReportLab.pdfbase.pdfmetrics import getFont as pdfmetrics_getFont, stringWidth as pdfmetrics_stringWidth, unicode2T1 as pdfmetrics_unicode2T1
from RTLReportLab.pdfbase.ttfonts import ShapedStr, ShapeData, _sdGuardL, shapeStr
...
try:
    try:
        ...
        from RTLReportLab import rlbidi
    ...
    from RTLReportLab.lib.utils import KlassStore
    ...
    def bidiFragWord(w,direction=None,bidiV=-1,bidiL=-1, clean=True):
        ...
        if not fsL:
            ...
            # Return the original word unchanged to avoid a [width-only] list that
            # would crash the caller with "TypeError: 'int' object is not subscriptable".
            return w
...
class PDFTextObject(_PDFColorSetter):
    ...
    def setRise(self, rise):
        ...
        v = f'{fp_str(rise)} Ts'
        if self._code and self._code[-1].endswith(' Ts'): #optimize out r0 Ts r1 Ts
