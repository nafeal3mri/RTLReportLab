...
import codecs, re
from RTLReportLab.lib.utils import isSeq, isBytes, isStr
...
class _isListOfShapes(Validator):
    ...
    def test(self, x):
        from RTLReportLab.graphics.shapes import Shape
        if isSeq(x):
            answer = 1
            for e in x:
...
from RTLReportLab.lib.normalDate import NormalDate
...
class _isValidChild(Validator):
    ...
    def test(self, x):
        ...
        from RTLReportLab.graphics.shapes import UserNode, Shape
...
class OneOf(Validator):
    ...
    Usage:
    f = RTLReportLab.lib.validators.OneOf('happy','sad')
    or
    f = RTLReportLab.lib.validators.OneOf(('happy','sad'))
